...
def _ulower(value):
    ...
    through to the catalogue, its sittings list came back empty, search could
...
def connect():
    """
    <summary>
    Open the mirror database, registering the ULOWER function and applying the schema and rating columns once per process.
    </summary>
...
def _provider(item, name):
    """
    <summary>
    One provider id from a Jellyfin item as a string, or None.
    </summary>
    <param name="item">A Jellyfin item.</param>
    <param name="name">Imdb, Tmdb or Tvdb.</param>
    <returns>The id, stripped, or None.</returns>
    """
...
def _numeric(value):
    """
    <summary>
    A float from a value, or None when absent or not numeric.
    </summary>
    <param name="value">Any value.</param>
    <returns>The float or None.</returns>
    """
    try:
