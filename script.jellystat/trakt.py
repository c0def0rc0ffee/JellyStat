...
class Index(object):
    ...
    def __init__(self):
        """
        <summary>
        Build every lookup table from the mirror in one pass.
        </summary>
        <remarks>
        By IMDb, TMDb and TVDb id, by normalised title (films with and without year), by show title, and by show, season and episode number, plus runtimes.
        </remarks>
        """
...
def _apply_runtime(session, item):
    """
    <summary>
    Copy an item's runtime into a session as both runtime and watched time, marking the runtime known.
    </summary>
    <param name="session">A parsed session.</param>
