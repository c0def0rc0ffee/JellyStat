...
def norm(text):
    """
    <summary>
    Lowercase letters and digits only, the form names are compared in.
    </summary>
    <param name="text">Any text, None allowed.</param>
...
def now():
    """
    <summary>
    The current time in the play log's stamp format.
    </summary>
...
def _row_to_list(row):
    """
    <summary>
    A lists table row, with its two counts, as the dict the API returns.
    </summary>
    <param name="row">id, name, description, source, created_at, updated_at, count, matched.</param>
...
def rename(list_id, name, description=None, connection=None):
    """
    <summary>
    Rename a list, optionally changing its description.
    </summary>
    <param name="list_id">The list.</param>
    <param name="name">New name, stripped.</param>
    <param name="description">New description, or None to leave it alone.</param>
    <param name="connection">A caller's connection, or None to open and close one here.</param>
    <exception cref="ListError">An empty name, a name over MAX_NAME, or a name already in use.</exception>
    """
    name = (name or "").strip()
    if not name:
        raise ListError("A list needs a name.")
...
def _touch(connection, list_id):
    """
    <summary>
    Stamp a list's updated_at with the current time.
    </summary>
    <param name="connection">An open connection, inside the caller's transaction.</param>
    <param name="list_id">The list.</param>
    """
...
def remove(list_id, entry_id, connection=None):
    ...
    <param name="list_id">The list.</param>
    <param name="entry_id">The entry row id.</param>
    <param name="connection">A caller's connection, or None to open and close one here.</param>
    <returns>True when a row was removed; the list is touched only then.</returns>
    """
    owned = connection is None
    connection = connection or connect()
    try:
...
def clear(list_id, connection=None):
    """
    <summary>
    Delete every entry of a list and touch it.
    </summary>
    <param name="list_id">The list.</param>
    <param name="connection">A caller's connection, or None to open and close one here.</param>
    """
