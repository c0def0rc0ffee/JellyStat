...
the one thing the mirror deliberately does not hold. The full catalogue is
...
sync, rather than a film lagging behind the hourly catalogue cache while a
...
If Jellyfin is unreachable the catalogue cannot refresh; with include_seen
...
# ---------------------------------------------------------------------------
# Catalogue (watched and unwatched, from Jellyfin)
# ---------------------------------------------------------------------------

def _fetch(creds, item_type):
    """
    <summary>
    Every item of a type from Jellyfin, with the fields the recommender needs.
    </summary>
    <param name="creds">A credentials dict.</param>
    <param name="item_type">Movie or Series.</param>
...
def catalog():
    ...
    with _lock:
        ...
        log("Catalogue fetched: %d movies, %d series" % (len(movies),
                                                       len(series)))
...
def _mirror_rows(media):
    """
    <summary>
    genres, play_count and series_name for every mirror row of a medium.
    </summary>
    <param name="media">movie, episode or series.</param>
...
def _genres_of(raw_json):
    """
    <summary>
    Effective genres from a mirror row's stored JSON, Unknown when it will not parse.
    </summary>
    <param name="raw_json">The genres column.</param>
...
def watched_movies():
    ...
    the hour-long catalogue cache instead, so with "hide seen" on, a film
...
def _entry(item, seen, score, match, view, covered):
    """
    <summary>
    One recommendation row for the page.
    </summary>
    <param name="item">A Jellyfin item.</param>
    <param name="seen">Whether it has been watched.</param>
    <param name="score">The blended score.</param>
    <param name="match">The taste match part.</param>
    <param name="view">The opinion part.</param>
    <param name="covered">Whether an opinion was available.</param>
    <returns>A dict.</returns>
    <remarks>
    match and opinion are kept apart so the page can say why: matches what you watch and you rate this sort of thing well are different claims.
    </remarks>
    """
    return {
        "id": item.get("Id"),
...
def _seen_movie(item, watched=None):
    ...
    <remarks>
    `watched` is the live set from the mirror; the catalogue's own Played
...
def recommendations(media, include_seen, limit=RECOMMEND_LIMIT, offset=0,
    ...
    try:
        ...
        # Rewatch-only fallback: the mirror is the seen side of the catalogue.
...
def _clean(names):
    """
    <summary>
    A set of lowercased, stripped genre names, empties dropped.
    </summary>
    <param name="names">A list, or None.</param>
...
def _mirror_as_items(media):
    """
    <summary>
    Mirror rows reshaped as catalogue items, for the offline fallback.
    </summary>
    """
    if media == "movies":
...
def search(term, limit=25):
    ...
    # Every mirror match, not the top `limit` of them. The catalogue merge
    ...
    # invisible here, so it came back from the catalogue marked never
...
def browse(media, limit=48, offset=0, genres=None, exclude=None,
    ...
    for row in rows:
        ...
        # from the catalogue above went through effective_genres and kept
...
def _shown(row, show):
    """
    <summary>
    Whether a library row belongs in the requested view.
    </summary>
    <param name="row">A browse row.</param>
    <param name="show">watched, unwatched, rated, unrated, favourites, duplicates, or anything else for all.</param>
...
def _narrow(rows, genres, exclude, year_from, year_to, rating_min):
    """
    <summary>
    Apply the genre, year and rating filters to library rows.
    </summary>
    <param name="rows">Browse rows.</param>
    <param name="genres">Every one of these must be present.</param>
    <param name="exclude">None of these may be present.</param>
    <param name="year_from">Lowest year, or falsy.</param>
    <param name="year_to">Highest year, or falsy.</param>
    <param name="rating_min">Lowest community rating, or falsy.</param>
    <returns>The rows that pass.</returns>
    """
    wanted = _clean(genres)
    unwanted = _clean(exclude)
    out = []
...
def _sort_browse(rows, sort, descending):
    """
    <summary>
    Sort library rows in place by the chosen column, name breaking ties so paging is stable.
    </summary>
    <param name="rows">Browse rows.</param>
    <param name="sort">name, year, rating, mine, plays, watched or runtime.</param>
    <param name="descending">True, False, or None for the column's natural way up.</param>
    """
...
def _similarity(target_genres, target_year, item):
    """
    <summary>
    How alike an item is to a target: genre overlap, with a nudge for a close year and a better rating.
    </summary>
    <param name="target_genres">Lowercased genres of the target.</param>
    <param name="target_year">The target's year, or None.</param>
    <param name="item">A Jellyfin item.</param>
    <returns>A float, 0 with no genre overlap.</returns>
    """
...
def copies_of(media, item_id, name, year):
    ...
    and the catalogue merged; the item page used to count them over the
    ...
    for item in (movies if media == "movie" else series):
        ...
        out.append({
            ...
            # the catalogue has none, and guessing at them would be worse
...
def catalog_detail(media, item_id=None, name=None):
    """
    <summary>
    A title's details straight from the catalogue, watched or not.
    </summary>
    <remarks>
