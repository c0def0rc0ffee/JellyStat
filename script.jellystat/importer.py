...
1. `stage(filename, raw)` - the file is parsed into normalised sessions and
   ...
   format, how many sessions, the date range, and the part that matters most,
...
def _too_short(session, min_seconds):
    """
    <summary>
    Whether a session was watched for less than the live logger's minimum, but for more than nothing.
    </summary>
    <param name="session">A session dict.</param>
    <param name="min_seconds">The threshold.</param>
    <returns>True when it is too short to count.</returns>
    """
...
def _session(started, media, title, **extra):
    """
    <summary>
    Build one play log session dict in the shape every parser hands on.
    </summary>
    <param name="started">Start time as a datetime.</param>
    <param name="media">movie or episode; anything else becomes other.</param>
    <param name="title">Title; a blank becomes a question mark.</param>
    <param name="extra">Optional fields: ended, watched_seconds, show, season, episode, year, runtime_seconds, device, dateonly.</param>
    <returns>The session dict, ended defaulting to started.</returns>
    """
...
def _int_or_none(value):
    """
    <summary>
    An int from anything, or None when it will not parse.
    </summary>
    <param name="value">Any value.</param>
    <returns>The integer or None.</returns>
    """
    try:
...
def _parse_jellystat_json(data):
    """
    <summary>
    Another box's JSON export: its plays list in the generic row shape.
    </summary>
    <param name="data">The decoded JSON document.</param>
...
def _parse_generic_rows(rows):
    """
    <summary>
    Rows in JellyStat's own export shape, one session per row with a parseable started_at.
    </summary>
    <param name="rows">Dicts with started_at, media, title and the optional fields.</param>
    <returns>A list of sessions.</returns>
    <remarks>
    A row whose stamp is a bare date is marked dateonly.
    </remarks>
    """
    sessions = []
    for row in rows:
...
def _prune():
    """
    <summary>
    Drop staged imports older than STAGE_TTL_S.
    </summary>
    """
...
def duplicate_of(session, existing_keys):
    """
    <summary>
    Whether a session matches something already in the log by title and day.
    </summary>
    <param name="session">A session dict.</param>
    <param name="existing_keys">The set from _existing_index().</param>
...
def _preview(session):
    """
    <summary>
    The few fields of a session the staging report shows.
    </summary>
    <param name="session">A session dict.</param>
    <returns>A small dict.</returns>
    """
    return {
...
def discard(token):
    """
    <summary>
    Forget a staged import.
    </summary>
    <param name="token">The staging token.</param>
...
def _batches_connect():
    """
    <summary>
    A play log connection with the import batches schema in place.
    </summary>
...
def batches():
    """
    <summary>
    Every import batch with its session count and date span, newest first.
    </summary>
    <returns>A list of dicts: id, imported_at, filename, format, mode, replaced, sessions, from, to.</returns>
    """
    connection = _batches_connect()
    try:
        rows = connection.execute(
