...
def _played_dates(items):
    """
    <summary>
    The parsed LastPlayedDate of every item that has one.
    </summary>
...
def _window(items, genre_lookup, per_day, avg_gap):
    """
    <summary>
    One time window of the payload: counts, pace and the three genre tallies.
    </summary>
    <param name="items">Items in the window.</param>
    <param name="genre_lookup">Series id to genres, for episodes.</param>
    <param name="per_day">Plays per day.</param>
    <param name="avg_gap">Average minutes between plays.</param>
...
def _media_block(all_time_items, recent_items, genre_lookup=None):
    """
    <summary>
    A medium's payload: the last 30 days window and the all time window.
    </summary>
    <param name="all_time_items">Everything watched.</param>
    <param name="recent_items">Watched within the window.</param>
    <param name="genre_lookup">Series id to genres, for episodes.</param>
    <returns>A dict.</returns>
    """
...
def build_payload(all_time_movies, recent_movies,
    ...
    """
    <summary>
    The whole snapshot the website receives, versioned and stamped.
    </summary>
    <param name="all_time_movies">Every watched film.</param>
    <param name="recent_movies">Films in the window.</param>
    <param name="all_time_episodes">Every watched episode.</param>
    <param name="recent_episodes">Episodes in the window.</param>
    <param name="series_genres">Series id to genres.</param>
    <returns>A dict.</returns>
    <remarks>
    windows duplicates movies so older site code keeps working.
    </remarks>
    """
...
def post(url, token, payload, ignore_ssl):
    """
    <summary>
    POST the payload and insist on an HTTP 200 with ok true.
    </summary>
    <param name="url">The endpoint.</param>
    <param name="token">X-Auth-Token value, or empty.</param>
    <param name="payload">The snapshot.</param>
    <param name="ignore_ssl">Skip TLS verification.</param>
    <exception cref="SendError">Any HTTP error, an unreachable site, a non JSON reply, or a reply that is not ok.</exception>
    <remarks>
    Sending with TLS verification off is logged as a warning, because anything on the path can read and alter the token and the payload.
    </remarks>
    """
