...
def _insert_history(connection, sessions, batch_id, skip_duplicates=True):
    """
    <summary>
    Insert parsed Trakt sessions as completed, assumed play log rows under a batch id.
    </summary>
    <param name="connection">An open connection, inside the caller's transaction.</param>
    <param name="sessions">Parsed sessions.</param>
    <param name="batch_id">The import batch.</param>
    <param name="skip_duplicates">Leave out sessions marked duplicate.</param>
    <returns>A tuple of added and skipped counts.</returns>
    """
    added = skipped = 0
