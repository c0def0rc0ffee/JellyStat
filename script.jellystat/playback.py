...
def _collect(method, key, id_field):
    """
    <summary>
    Map Jellyfin ids found in Kodi library file paths to the Kodi entry that holds them.
    </summary>
    <param name="method">VideoLibrary.GetMovies or VideoLibrary.GetEpisodes.</param>
    <param name="key">The result key those return.</param>
    <param name="id_field">movieid or episodeid.</param>
    <returns>Lowercased Jellyfin id to {type, dbid}.</returns>
    """
    found = {}
...
def _rpc_item(entry):
    """
    <summary>
    The item argument JSON-RPC wants for a resolved entry.
    </summary>
    <param name="entry">A {type, dbid} dict.</param>
    <returns>A one key dict.</returns>
    """
...
def _label(entry):
    """
    <summary>
    The Kodi title of a resolved entry, or the item when it cannot be read.
    </summary>
    <param name="entry">A {type, dbid} dict.</param>
...
def _seconds(clock):
    """
    <summary>
    Seconds from a JSON-RPC time dict, 0 for anything else.
    </summary>
    <param name="clock">A dict with hours, minutes and seconds.</param>
