...
def _addon():
    ...
    """
    <summary>
    The addon object by explicit id, since the service and the script resolve context differently.
    </summary>
...
class Handler(BaseHTTPRequestHandler):
    """
    <summary>
    One HTTP request against the dashboard: the page, the login, and the JSON API.
    </summary>
    <remarks>
    HTTP/1.1 with a per request timeout, so a client that announces a body and stalls cannot hold a thread.
    </remarks>
    """
    ...
    def log_message(self, fmt, *args):
        ...
        """
        <summary>
        Route the server's access log to Kodi's log at debug level.
        </summary>
        <param name="fmt">printf style format.</param>
    ...
    def _send(self, code, body, content_type, extra_headers=None):
        """
        <summary>
        Write one response with a no-store cache header, ignoring a browser that has gone away.
        </summary>
        <param name="code">HTTP status.</param>
        <param name="body">str or bytes.</param>
        <param name="content_type">Content-Type value.</param>
        <param name="extra_headers">(name, value) pairs, or None.</param>
        """
    ...
    def _password(self):
        """
        <summary>
        The configured dashboard password, empty when none.
        </summary>
    ...
    def do_GET(self):
        ...
        """
        <summary>
        Serve the page, the login page when a password is set and the session is missing, the favicon, or route an API GET.
        </summary>
        <remarks>
        The Host header is checked first, before the password: a request naming somebody else's domain is not this dashboard's traffic.
        </remarks>
        """
    ...
    def do_POST(self):
        """
        <summary>
        Route a POST: login first, then everything that changes state, behind the Host check, the cross origin check and the password.
        </summary>
        """
        if not _host_allowed(self.headers.get("Host")):
            self._reject_host()
    ...
    def _read_body(self, limit=importer.MAX_BYTES):
        """
        <summary>
        The request body, refusing and draining anything larger than the limit.
        </summary>
        <param name="limit">Bytes.</param>
        <returns>The body bytes.</returns>
        <exception cref="importer.ImportError_">When the announced length is over the limit.</exception>
        """
        length = int(self.headers.get("Content-Length") or 0)
        if length > limit:
    ...
    def _drain(self, length):
        """
        <summary>
        Read and discard a number of bytes so the client sees the answer rather than a broken pipe.
        </summary>
        <param name="length">Bytes to discard.</param>
        """
        remaining = length
        while remaining > 0:
    ...
    def _import_commit(self):
        """
        <summary>
        Write a staged file import under the chosen mode and invalidate the dashboard cache.
        </summary>
        """
        body = self._json_body()
        try:
    ...
    def _import_delete(self):
        """
        <summary>
        Remove an import batch and every session it added.
        </summary>
        """
    ...
    def _login(self):
        """
        <summary>
        Check the posted password and issue a session cookie that expires.
        </summary>
        <remarks>
        A malformed Content-Length answers 400 rather than dropping the connection. The cookie is a fresh random token good for SESSION_HOURS; the browser still treats it as a session cookie, but the server decides when it stops working.
        </remarks>
        """
        try:
            length = int(self.headers.get("Content-Length") or 0)
    ...
    def _serve_data(self, query):
        """
        <summary>
        The overview payload, rebuilt when stale or when refresh=1, recording today's snapshot when a rebuild produced one.
        </summary>
    ...
    def _serve_detail(self, media, item_id=None, name=None):
        """
        <summary>
        One title's page: from the mirror, else from the catalogue.
        </summary>
        <remarks>
        ...
        catalogue and mark it unwatched rather than answering "no such film".
        </remarks>
        """
    ...
    def _attach_copies(self, detail, media):
        """
        <summary>
        Widen the mirror's list of copies to the catalogue's, never failing.
        </summary>
        <remarks>
        ...
        while the list page, counting over the catalogue too, has already
    ...
    def _serve_suggestions(self, path, query):
        ...
        <remarks>
        Both need the Jellyfin catalogue for anything unseen, so both answer
    ...
    def _serve_list(self, query):
        """
        <summary>
        One list by id, or 400, 404 or 500.
        </summary>
        <param name="query">Parsed query parameters.</param>
        """
        try:
    ...
    def _serve_trakt_commit(self):
        """
        <summary>
        Import the chosen Trakt categories under a mode.
        </summary>
        """
        body = self._json_body()
        try:
    ...
    def _serve_history(self, query):
        """
        <summary>
        Snapshot totals and genre history for a medium.
        </summary>
        <param name="query">Parsed query parameters; media defaults to movies.</param>
        """
        media = (query.get("media") or ["movies"])[0]
        if media not in ("movies", "tv"):
            media = "movies"
...
def is_running():
    """
    <summary>
    Whether the dashboard server is up.
    </summary>
...
def current_port():
    """
    <summary>
    The port the dashboard is listening on, or None.
    </summary>
    <returns>An int or None.</returns>
    """
...
def stop():
    """
    <summary>
    Shut the dashboard down, logging rather than raising if it will not close cleanly.
    </summary>
    """
    global _server, _thread
    if _server is None:
