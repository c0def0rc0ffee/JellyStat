...
def connect():
    ...
    """
    <summary>
    Open the play log database, applying the schema, migrations and one time repairs once per process.
    </summary>
    <returns>An sqlite3 connection.</returns>
    <remarks>
    A failed repair is logged and retried at the next start rather than stopping the log opening.
    </remarks>
    """
    global _schema_done
...
def _rows(query, params=()):
    """
    <summary>
    Run a read query and return its rows, or an empty list after logging a failure.
    </summary>
    <param name="query">SQL.</param>
    <param name="params">Bound parameters.</param>
...
def _kind(media):
    """
    <summary>
    The chart bucket for a media type.
    </summary>
    <param name="media">movie, episode or other.</param>
    <returns>movies, episodes or None.</returns>
    """
    if media == "movie":
        return "movies"
    if media == "episode":
